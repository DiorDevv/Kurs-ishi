...
def show_auth_interface(root, protocol, back_command, home_command, bg_color, button_color, text_color, accent_color):
    ...
    def kerberos_auth():
        try:
            ...
            result_text.insert(tk.END, "─" * 70 + "\n")
            result_text.insert(tk.END,
                               f"   Session Key (Service): {binascii.hexlify(K_session_service).decode()[:32]}...\n")
            ...
            result_text.insert(tk.END,
                               f"   Service Ticket: {binascii.hexlify(encrypted_service_ticket).decode()[:40]}...\n\n")
